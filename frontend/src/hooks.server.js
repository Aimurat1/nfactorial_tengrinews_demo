...
export const handle = async ({ event, resolve }) => {
    ...
    event.fetch = async (resource, init) => {
        ...
        if (typeof resource === 'string' && (resource.startsWith('http://localhost:8000') || resource.startsWith('http://127.0.0.1:8000')) ) {
            // Modify the URL here if it matches your criteria
            resource = resource.replace('http://localhost:8000', 'http://backend:8000');
            // resource = resource.replace('http://127.0.0.1:8000', 'http://backend:8000');

        } else if (resource instanceof Request && (resource.url.startsWith('http://localhost:8000') || resource.url.startsWith('http://127.0.0.1:8000'))) {
